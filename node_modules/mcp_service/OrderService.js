...
OrderService.prototype.incSuccessTicketCount = function(orderId, cb)
{
    ...
    var cols = {id:1, termCode:1, schemeId:1, gameCode:1, outerId:1, amount:1,
        ...
        multiple:1, numbers:1, version:1, printCount:1, printFailCount:1};
    transaction.run(function(wCb){
        async.waterfall([
            //find the order
            function(cb){
                var cursor = orderTable.find({id:orderId}, cols);
                cursor.dateToString();
            ...
            function(torder, cb)
            {
                ...
                if(torder.printCount + torder.printFailCount >= torder.ticketCount)
                {
                    ...
                    torder.printTime = dateUtil.getCurTime();
                }
                var data = {$inc:{printCount:1}, $set:{status:torder.status}};
                if(torder.printTime)
                {
                    data.$set.printTime = torder.printTime;
                }
